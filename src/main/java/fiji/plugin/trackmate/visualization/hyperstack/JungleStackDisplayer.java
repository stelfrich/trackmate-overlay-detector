...
/**
 * A TrackMate displayer that has a compatible behavior with the assumption that
 * {@link Roi}s in an overlay need not be cleared.
 * 
 * @author Stefan Helfrich
 */
public class JungleStackDisplayer extends AbstractTrackMateModelView
{
	...
	protected JungleSpotOverlay spotOverlay;

	protected TrackOverlay trackOverlay;

	private JungleTool editTool;

	private Roi initialROI;

	public static final String KEY = "JUNGLESTACKDISPLAYER";

	/*
	 * CONSTRUCTORS
	 */

	public JungleStackDisplayer( final Model model, final SelectionModel selectionModel, final ImagePlus imp )
	{
	...
	public JungleStackDisplayer( final Model model, final SelectionModel selectionModel )
	{
	 ...
	 */
	protected JungleSpotOverlay createSpotOverlay()
	{
		return new JungleSpotOverlay( model, imp, displaySettings );
	}
	...
	{
		editTool = JungleTool.getInstance();
		if ( !JungleTool.isLaunched() )
			editTool.run( "" );
		else
		{
